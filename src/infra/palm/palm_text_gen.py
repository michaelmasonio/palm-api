...
def generate_close_to(answer):
    prompt = f"Please provide a similar but different statement for embeddings: {answer}"
    models = [
        m
        for m in palm.list_models()
        if "generateText" in m.supported_generation_methods
...
def generate_not_close_to(answer):
    prompt = f"Please provide a dissimilar statement for embeddings: {answer}"
    models = [
        m
        for m in palm.list_models()
        if "generateText" in m.supported_generation_methods
